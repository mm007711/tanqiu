(() => {
  ...
  function drawOverlayV2() {
    ...
    if (paused) {
      ...
      text("1 / 2 / 3 切换锚型，4 / 5 / 6 / 7 布置木板", FIELD.x + FIELD.w / 2, oy + 158, 13, "#cdd8d4", "center");
    }
    ctx.restore();
  }
  ...
  function drawBottomControlsV2() {
    ...
    const items = isNetworkNavigator()
      ...
      : [
        ...
        [MP.enabled ? "联机" : "4 / 5 / 6 / 7", MP.enabled ? MP.room : "布板", MP.enabled ? palette.brass : state.navigatorCooldown <= 0 ? palette.brass : palette.gray]
      ];
    let cx = x + 18;
  ...
  function addDirectionalParticles(x, y, count, angle, spread = 0.55, speed = 120, color = palette.foam, size = 3) {
    for (let i = 0; i < count; i++) {
  ...
  function addBoardMountBurst(b) {
    const mounts = boardMountWorldPoints(b);
    addRing(b.x, b.y, Math.max(b.w, b.h) * 0.42, b.accent, 0.22);
    addWaterRipple(b.x, b.y, 0.9);
    addDirectionalParticles(b.x, b.y, 8, b.angle, 0.7, 120, "#b78951", 2.5);
    for (const m of mounts) {
      addRing(m.x, m.y, 20, b.accent, 0.24);
      addParticles(m.x, m.y, 7, 105, palette.brass, 2.4);
      addWaterRipple(m.x, m.y, 0.42);
    }
  }

  function addBoardBreakBurst(b, source = "ball") {
    const mounts = boardMountWorldPoints(b);
    const heavy = source === "heavy" || b.kind === "blast";
    const count = heavy ? 6 : 5;
    for (let i = 0; i < count; i++) {
      const t = count === 1 ? 0.5 : i / (count - 1);
      const lx = lerp(-b.w * 0.45, b.w * 0.45, t);
      const p = boardWorldVec(b, lx, rnd(-b.h * 0.25, b.h * 0.25));
      const px2 = b.x + p.x;
      const py2 = b.y + p.y;
      addDirectionalParticles(px2, py2, heavy ? 7 : 5, b.angle + (i % 2 ? Math.PI / 2 : -Math.PI / 2), 0.85, heavy ? 220 : 165, i % 2 ? b.accent : "#b78951", heavy ? 3.3 : 2.8);
    }
    for (const m of mounts) {
      addParticles(m.x, m.y, heavy ? 12 : 8, heavy ? 190 : 130, palette.brass, 2.7);
      addRing(m.x, m.y, heavy ? 30 : 22, b.accent, 0.32);
    }
    addRing(b.x, b.y, Math.max(b.w, b.h) * (heavy ? 0.72 : 0.58), b.accent, heavy ? 0.5 : 0.38);
    if (heavy) addRing(b.x, b.y, Math.max(b.w, b.h) * 0.95, palette.coral, 0.42);
  }
  ...
  function canNavigatorControl() {
    return !MP.enabled || MP.role === "navigator" || MP.role === "versus";
  }
  ...
  function boardPlacementStatus(card = null) {
    if (state.gameOver) return { ok: false, label: "航程结束", color: palette.gray };
    if (state.navigatorCooldown > 0) return { ok: false, label: `冷却 ${state.navigatorCooldown.toFixed(1)}s`, color: palette.gray };
    if (availableBoardCount() >= BOARD_LIMIT) return { ok: false, label: "航障已满", color: palette.coral };
    if (card && state.navigatorMaterial < card.cost) return { ok: false, label: `${resourceLabel()}不足`, color: palette.gray };
    const cheapest = Math.min(...Object.values(BOARD_CARDS).map(c => c.cost));
    if (!card && state.navigatorMaterial < cheapest) return { ok: false, label: `${resourceLabel()}不足`, color: palette.gray };
    return { ok: true, label: "可挂板 4-7", color: palette.blue };
  }
  ...
  function playNavigatorCard(kind, source = "local") {
    ...
    const remoteRequest = source && source !== "local" && source !== "remote";

    if (MP.enabled && (MP.role === "navigator" || MP.role === "versus") && source === "local") {
      if (!MP.connected) {
        setMessage("中继未连接，无法布场", 0.85);
        addFloat("未连接", FIELD.x + FIELD.w / 2, FIELD.y + 92, palette.gray, 14);
        beep(130, 0.045, 0.016, "triangle");
        return false;
      }
      if (!MP.roles.helm) {
        setMessage("等待掌舵手进入房间", 0.85);
        addFloat("等待掌舵手", FIELD.x + FIELD.w / 2, FIELD.y + 92, palette.gray, 14);
        beep(130, 0.045, 0.016, "triangle");
        return false;
      }
      const cardId = `${MP.room}:${MP.role}:${++MP.cardSeq}`;
      if (!sendNet({ type: "play_card", kind, cardId })) {
        setMessage("布场请求发送失败", 0.85);
        return false;
      }
    ...
    setMessage(`${boardCardName(card)}：挂到钉点，${card.hint}`, 1.05);
    addFloat(spec.mountSpan ? "钉点预挂" : boardCardName(card), spec.x, spec.y - 42, card.accent, 15, -24);
    ...
    sendAuthoritativeSnapshot("card");
    return true;
  }

  function boardMountAngle(angle) {
    let a = angle;
  ...
  function boardMountAngleDiff(a, b) {
    return Math.abs(boardMountAngle(a - b));
  }

  function canUsePinAsBoardMount(p, card) {
    if (!p || p.broken || !p.anchorable) return false;
    if (p.kind === "pin" || p.kind === "brass" || p.kind === "tide") return true;
    return p.kind === "anchor" && (card.key === "guard" || card.key === "block");
  }

  function boardMountGoal(card, x, y, angle) {
    const goal = {
      x,
      y,
      angle,
      idealSpan: Math.max(72, card.w - 18),
      radius: 230,
      angleWeight: 0.6,
      angleTolerance: 0.62
    };
    if (card.key === "guard") {
      ...
      goal.idealSpan = 112;
      goal.radius = 260;
      goal.angleWeight = 0.95;
      goal.angleTolerance = 0.38;
    } else if (card.key === "ramp") {
      goal.idealSpan = 96;
      goal.radius = 235;
      goal.angleWeight = 0.9;
      goal.angleTolerance = 0.5;
    } else if (card.key === "blast") {
      goal.angle = null;
      goal.idealSpan = 82;
      goal.radius = 250;
      goal.angleWeight = 0.15;
    } else if (card.key === "block") {
      goal.angle = 0;
      goal.idealSpan = 90;
      goal.radius = 260;
      goal.angleWeight = 0.75;
      goal.angleTolerance = 0.48;
    }
    return goal;
  }

  function boardMountConflict(p1, p2, x, y, angle) {
    const a = Math.min(p1.id, p2.id);
    const b = Math.max(p1.id, p2.id);
    const placed = driftBoards.concat(boardPreviews);
    for (const bd of placed) {
      if (bd.broken) continue;
      const ba = Math.min(bd.pinAId ?? -1, bd.pinBId ?? -2);
      const bb = Math.max(bd.pinAId ?? -1, bd.pinBId ?? -2);
      if (ba === a && bb === b) return true;
      if (dist(x, y, bd.x, bd.y) < 48 && boardMountAngleDiff(angle, bd.angle || 0) < 0.38) return true;
    }
    return false;
  }

  function boardMountClearanceOk(card, p1, p2, x, y, angle, w, h) {
    const c = Math.cos(angle);
    const s = Math.sin(angle);
    const ex = Math.abs(c) * w * 0.5 + Math.abs(s) * h * 0.5 + 10;
    const ey = Math.abs(s) * w * 0.5 + Math.abs(c) * h * 0.5 + 10;
    if (x - ex < FIELD.x + 18 || x + ex > FIELD.r - 104) return false;
    if (y - ey < FIELD.y + 96 || y + ey > FIELD.b - 74) return false;
    if (!ball.dead && pointSegmentDistance(ball.x, ball.y, p1.x, p1.y, p2.x, p2.y) < ball.r + h + 20) return false;
    for (const b of bumpers) {
      if (pointSegmentDistance(b.x, b.y, p1.x, p1.y, p2.x, p2.y) < b.r + 18) return false;
    }
    for (const p of pockets) {
      const clear = card.key === "block" ? p.r + 14 : p.r + 24;
      if (dist(x, y, p.x, p.y) < clear) return false;
    }
    return true;
  }

  function scoreBoardMountPair(card, p1, p2, goal) {
    const span = dist(p1.x, p1.y, p2.x, p2.y);
    const minSpan = Math.max(54, card.w * (card.key === "blast" ? 0.72 : 0.62));
    const maxSpan = card.w + (card.key === "guard" ? 26 : card.key === "block" ? 24 : 20);
    if (span < minSpan || span > maxSpan) return null;

    const x = (p1.x + p2.x) * 0.5;
    const y = (p1.y + p2.y) * 0.5;
    const angle = boardMountAngle(Math.atan2(p2.y - p1.y, p2.x - p1.x));
    const w = Math.round(clamp(span + 18, card.w, maxSpan));
    if (boardMountConflict(p1, p2, x, y, angle)) return null;
    if (!boardMountClearanceOk(card, p1, p2, x, y, angle, w, card.h)) return null;

    const spanScore = 1 - clamp(Math.abs(span - goal.idealSpan) / 54, 0, 1);
    const targetScore = 1 - clamp(dist(x, y, goal.x, goal.y) / goal.radius, 0, 1);
    let score = spanScore * 0.85 + targetScore * 1.25;

    if (goal.angle !== null) {
      const angleScore = 1 - clamp(boardMountAngleDiff(angle, goal.angle) / goal.angleTolerance, 0, 1);
      score += angleScore * goal.angleWeight;
    }
    if (card.key === "guard") {
      score += clamp((y - (FIELD.b - 260)) / 190, 0, 1) * 0.5;
      score += (1 - clamp(Math.abs(angle) / 0.36, 0, 1)) * 0.35;
    } else if (card.key === "ramp") {
      score += (1 - clamp(Math.abs(Math.abs(angle) - 0.43) / 0.38, 0, 1)) * 0.45;
    } else if (card.key === "blast") {
      score += (y > FIELD.y + 165 && y < FIELD.b - 165 ? 0.35 : 0);
    } else if (card.key === "block") {
      score += clamp((y - (FIELD.b - 260)) / 190, 0, 1) * 0.6;
      score += (1 - clamp(Math.abs(angle) / 0.46, 0, 1)) * 0.45;
    }
    if (p1.kind === "brass" || p2.kind === "brass") score += 0.08;
    if (p1.kind === "tide" || p2.kind === "tide") score += 0.06;
    if (p1.kind === "anchor" || p2.kind === "anchor") score += card.key === "block" ? 0.05 : -0.08;
    score += hash2(p1.id, p2.id, state.boardSeq) * 0.035;

    return { p1, p2, x, y, angle, w, span, score };
  }

  function findBoardMountPair(card, goal) {
    const mountPins = pins.filter(p => canUsePinAsBoardMount(p, card));
    let best = null;
    for (let i = 0; i < mountPins.length; i++) {
      for (let j = i + 1; j < mountPins.length; j++) {
        const candidate = scoreBoardMountPair(card, mountPins[i], mountPins[j], goal);
  ...
  function boardMountWorldPoints(b) {
    if (b.mountA && b.mountB) return [b.mountA, b.mountB];
    if (!b.mountSpan) return [];
    const dx = Math.cos(b.angle) * b.mountSpan * 0.5;
    const dy = Math.sin(b.angle) * b.mountSpan * 0.5;
    return [{ x: b.x - dx, y: b.y - dy }, { x: b.x + dx, y: b.y + dy }];
  }
  ...
  function makeBoardSpec(card) {
    ...
    const mount = findBoardMountPair(card, boardMountGoal(card, x, y, angle));
    const mountData = mount ? {
      pinAId: mount.p1.id,
      pinBId: mount.p2.id,
      mountSpan: mount.span,
      mountA: { x: mount.p1.x, y: mount.p1.y, kind: mount.p1.kind },
      mountB: { x: mount.p2.x, y: mount.p2.y, kind: mount.p2.kind },
      mountScore: mount.score
    } : {};
    if (mount) {
      x = mount.x;
      y = mount.y;
  ...
  function spawnBoard(preview) {
    ...
    addBoardMountBurst(board);
    addFloat("挂板锁定", board.x, board.y - 38, board.accent, 14, -24);
    beep(520, 0.04, 0.018, "square");
  }
  ...
  function breakBoard(b, source = "ball") {
    ...
    addBoardBreakBurst(b, source);
    addParticles(b.x, b.y, b.kind === "blast" ? 24 : 12, b.kind === "blast" ? 210 : 150, b.accent, 3.2);
    addParticles(b.x, b.y, 12, 180, "#b78951", 2.6);
  ...
  function drawBoardPreviews() {
    for (const b of boardPreviews) {
      ...
      ctx.save();
      drawBoardMountHardware(b, b.accent, true);
      ctx.translate(b.x, b.y);
      ctx.rotate(b.angle);
      ctx.globalAlpha = 0.18 + 0.2 * Math.sin(t * Math.PI);
      drawBoardPixelHull(b, b.accent, palette.foam, true);
  ...
  function drawBoardPixelHull(b, fill, stroke, preview = false, detail = true) {
    const x = Math.round(-b.w / 2);
    const y = Math.round(-b.h / 2);
    const w = Math.round(b.w);
    const h = Math.round(b.h);
    const cap = Math.max(8, Math.round(h * 0.48));
    const notch = Math.max(3, Math.round(h * 0.18));
    px(x + cap, y, w - cap * 2, h, fill);
    px(x, y + notch, cap, h - notch * 2, fill);
    px(x + w - cap, y + notch, cap, h - notch * 2, fill);
    if (detail) {
      px(x + cap + 2, y + 2, Math.max(0, w - cap * 2 - 4), 2, preview ? stroke : "#d0a061");
      px(x + cap + 2, y + h - 4, Math.max(0, w - cap * 2 - 4), 2, preview ? stroke : "#5b3a24");
    }
    if (stroke) {
      px(x + cap, y - 2, w - cap * 2, 2, stroke);
      px(x + cap, y + h, w - cap * 2, 2, stroke);
      px(x - 2, y + notch, 2, h - notch * 2, stroke);
      px(x + w, y + notch, 2, h - notch * 2, stroke);
      px(x, y + notch - 2, cap, 2, stroke);
      px(x, y + h - notch, cap, 2, stroke);
      px(x + w - cap, y + notch - 2, cap, 2, stroke);
      px(x + w - cap, y + h - notch, cap, 2, stroke);
    }
  }

  function drawBoardPixelDetails(b, cracked, critical) {
    const x = Math.round(-b.w / 2);
    const y = Math.round(-b.h / 2);
    const w = Math.round(b.w);
    const h = Math.round(b.h);
    const plankH = Math.max(4, Math.floor((h - 6) / 2));
    px(x + 10, y + 5, w - 20, plankH, "rgba(238,196,126,0.18)");
    px(x + 10, y + h - 5 - plankH, w - 20, plankH, "rgba(34,20,12,0.22)");
    for (let i = x + 20; i < x + w - 14; i += 18) {
      px(i, y + 4, 2, h - 8, "#3b291e");
      px(i + 3, y + 6, 1, Math.max(2, h - 12), "rgba(255,226,160,0.22)");
    }
    const pegColor = critical ? palette.brass : b.accent;
    px(x + 8, y + Math.floor(h / 2) - 2, 4, 4, pegColor);
    px(x + w - 12, y + Math.floor(h / 2) - 2, 4, 4, pegColor);
    px(x + Math.floor(w / 2) - 2, y + 3, 4, 3, "#2b1b12");
    px(x + Math.floor(w / 2) - 2, y + h - 6, 4, 3, "#2b1b12");
    if (cracked) {
      px(x + Math.floor(w * 0.32), y + 2, 3, Math.floor(h * 0.48), b.accent);
      px(x + Math.floor(w * 0.32) + 3, y + Math.floor(h * 0.42), Math.floor(w * 0.2), 3, b.accent);
      px(x + Math.floor(w * 0.58), y + Math.floor(h * 0.54), 3, Math.floor(h * 0.36), b.accent);
    }
  }

  function drawBoardMountHardware(b, color, preview = false) {
    const mounts = boardMountWorldPoints(b);
    if (mounts.length !== 2) return;
    ctx.save();
    ctx.globalAlpha = preview ? 0.42 : 1;
    line(mounts[0].x, mounts[0].y, mounts[1].x, mounts[1].y, preview ? palette.foam : "#5b3a24", preview ? 2 : 3);
    for (const m of mounts) {
      px(m.x - 7, m.y - 7, 14, 14, preview ? "rgba(246,247,238,0.42)" : "#2a1a12");
      px(m.x - 4, m.y - 4, 8, 8, preview ? color : palette.brass);
      px(m.x - 2, m.y - 2, 4, 4, palette.black);
      if (!preview) {
        px(m.x - 9, m.y - 2, 4, 4, color);
        px(m.x + 5, m.y - 2, 4, 4, color);
      }
    }
    ctx.restore();
  }
  ...
  function drawBoard(b) {
    ...
    drawBoardMountHardware(b, critical ? palette.brass : b.accent);

    ctx.save();
    ctx.translate(Math.round(b.x), Math.round(b.y));
    ctx.rotate(b.angle);
    drawBoardPixelHull(b, fill, critical ? palette.brass : palette.black);
    drawBoardPixelDetails(b, cracked, critical);

    if (flash > 0.1) {
      ctx.globalAlpha = 0.22 * flash;
      drawBoardPixelHull({ ...b, w: b.w + 8, h: b.h + 8 }, "rgba(0,0,0,0)", b.accent, false, false);
      ctx.globalAlpha = 1;
    }

    // 倒计时条
    const barW = Math.round(b.w * 0.76);
    const barY = Math.round(b.h / 2 - 6);
    px(-barW / 2, barY, barW, 3, "#2a1a12");
    px(-barW / 2, barY, Math.round(barW * lifePct), 3, b.accent);
  ...
  function drawNavigatorHud() {
    ...
    ctx.globalAlpha = 1;
    text(MP.enabled ? `联机 ${MP.role === "helm" ? "掌舵手" : "航海士"} · ${MP.room}` : "单机布场", x + 10, y + 8, 12, palette.foam, "left");
    text(MP.enabled ? MP.status : "4-7 放置木板", x + w - 10, y + 8, 10, MP.connected ? palette.blue : palette.brass, "right");
    ...
    const placementStatus = boardPlacementStatus();
    const cd = MP.enabled ? (state.navigatorCooldown > 0 ? `冷却 ${state.navigatorCooldown.toFixed(1)}s` : "可布场") : placementStatus.label;
    text(cd, x + 10, infoY + 20, 11, MP.enabled ? (state.navigatorCooldown > 0 ? palette.gray : palette.blue) : placementStatus.color, "left");
    text(`航障 ${availableBoardCount()}/${BOARD_LIMIT}`, x + w - 10, infoY + 20, 11, palette.foam, "right");
    if (!MP.enabled) {
      const statusY = infoY + 32;
      const statusW = w - 20;
      rect(x + 10, statusY, statusW, 8, "#243337", palette.black, 1);
      const fillW = placementStatus.ok ? statusW : Math.round(statusW * clamp(1 - state.navigatorCooldown / 2.0, 0, 1));
      px(x + 10, statusY, fillW, 8, placementStatus.color);
    }
    ...
    for (const [key, card] of cards) {
      const cardStatus = boardPlacementStatus(card);
      const affordable = cardStatus.ok;
      rect(cx, infoY + 42, 52, 26, affordable ? "#22343a" : "#182226", affordable ? card.accent : "#3d4a4e", 1);
      if (affordable && !MP.enabled) {
        ctx.globalAlpha = 0.16 + 0.1 * Math.sin(state.phase * 5 + Number(key));
        rect(cx + 3, infoY + 45, 46, 20, card.accent, null, 1);
        ctx.globalAlpha = 1;
      }
  ...
  function applyNetworkSnapshotMessage(msg) {
    if (!msg || !msg.snapshot) return false;
    if (msg.snapshotId && msg.snapshotId <= MP.lastSnapshotId && msg.reason !== "request") return false;
  ...
  function sendNet(payload) {
    if (!MP.enabled || !MP.ws || MP.ws.readyState !== WebSocket.OPEN) return false;
    MP.ws.send(JSON.stringify({ room: MP.room, role: MP.role, ...payload }));
    return true;
  }

  function sendAuthoritativeSnapshot(reason = "") {
    if (!MP.enabled || !MP.connected || MP.role !== "helm") return false;
    return sendNet({ type: "snapshot", snapshot: snapshotGame(), snapshotId: ++MP.snapshotSeq, reason });
  }
    ...
    try {
      ...
      MP.ws.addEventListener("message", event => {
        ...
        if (msg.type === "joined" || msg.type === "room_state" || msg.type === "peers") {
          ...
          applyNetworkSnapshotMessage(msg);
        } else if (msg.type === "request_snapshot" && MP.role === "helm") {
          sendAuthoritativeSnapshot("request");
  ...
  function updateNetwork(dt) {
    ...
    if (MP.role === "helm" && MP.connected) {
        ...
        if (MP.lastSnapshot >= MP.snapshotEvery) {
          MP.lastSnapshot = 0;
        sendAuthoritativeSnapshot("tick");
      }
    }
  ...
  window.__anchorProtoTest = {
    ...
    constants: {
      ...
      boardSpawnWarning: BOARD_SPAWN_WARNING,
    ...
    boardMountWorldPoints,
    boardPlacementStatus,
    ...
    setRole(role, connected = false) {
      ...
      MP.role = role === "solo" ? "helm" : role;
      MP.room = "test";
      MP.connected = connected;
      MP.roles = {};
      MP.pendingCards.length = 0;
      MP.ws = connected ? { readyState: WebSocket.OPEN, sent: [], send(raw) { this.sent.push(raw); } } : null;
    },
